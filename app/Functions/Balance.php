...
class Balance extends Controller
{
   function saldo()
   {
      ...
      //fee downline
      $fee = $this->db(0)->get_cols_where("balance", "SUM(amount) as amount", "user_id = '" . $log['user_id'] . "' AND flow = 1 AND (balance_type BETWEEN 22 AND 23) AND tr_status <> 2", 0)['amount'] ?? 0;

      $saldo = ($deposit + $fee + $invest_wd) - ($invest + $wd);
      return $saldo;
   }
}
